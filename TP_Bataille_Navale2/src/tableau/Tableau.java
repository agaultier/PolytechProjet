...
out.println("");  
	...
	{ //Pour qu'un joueur donne une case cible
		...
		try {
			...
			if (x>=25 || x<10 || y<0 || y>9 || cibleString.length()>2 ||cibleString.length()== 0) {
				outTir.println("Rentrez seulement deux charactères compris entre A et O, 0 et 9");
			}
			else{
				bonneEntree = true;
	...
	public static int[] positionnerBateau(BufferedReader in, PrintWriter out, int[][] tableauJoueur, int[] bateaux )
	{
		...
		bonneEntree = false;
			...
			while (bonneEntree == false) {
				out.println("Entrez les coordonnées, comprise entre A et O, 0 et 9");
				String cibleString;
				try {
			...
			if (x>=25 || x<10 || y<0 || y>9 || cibleString.length()!=2 ||  tableauJoueur[y][x-10]==1) {
				out.println("Rentrez seulement deux charactères compris entre A et 0, 0 et 9\nNe mettez pas un bateau sur une case que vous avez déjà choisi");
			}
			else{
